...
1. **搜索结果含 `<img>` 标签**：直接用 `img@src` 提取
2. **搜索结果无封面，详情页封面 URL 有规律**：用 JS 拼接封面 URL
...
`cover()` 函数从书籍 URL（如 `/426/`）提取 ID，按站点封面 URL 规律拼接。`result` 是 `@js:` 前规则（`a@href`）的结果。

**注意**：不同页面（首页、详情页、搜索页）封面域名和路径可能不同。首页/详情页用 `img.80ge.info`，详情页直接用 `.block_img2 img@src` 更可靠。

## 正文分页与清理

正文页含"下一页"链接时，必须添加 `nextContentUrl` 规则，否则只能拿到第一页内容：
...
- `nextContentUrl` 使用 CSS 选择器（如 `#pt_next@href`、`text.下一页@href`）提取下一页链接
- `content` 中的 `##本章未完，请点击下一页继续阅读##` 替换规则清除提示文字

**正文分页检测方法**：
1. 检查 HTML 中是否有"下一页"链接
2. 检查内容是否含"本章未完"等提示文字
3. 访问第二页 URL 确认结构一致
