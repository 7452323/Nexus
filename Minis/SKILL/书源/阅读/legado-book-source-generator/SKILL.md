...
### 封面提取
- 搜索结果无 `<img>` 标签时，用 `a@href@js:cover(result)` + `jsLib` 定义 `cover()` 函数拼接封面 URL。详见 `references/legado-source-behavior.md`。
- 不同页面（首页、详情页、搜索页）封面域名和路径可能不同。详情页用 `img@src` 直接提取最可靠。

### 正文分页
- 正文页含"下一页"链接时，`nextContentUrl` 必须使用 CSS 选择器（如 `#pt_next@href`、`text.下一页@href`），不要用变量引用（如 `defaultContentUrl`）。详见 `references/legado-source-behavior.md`。
- 正文含"本章未完，请点击下一页继续阅读"等提示文字时，用 `##本章未完，请点击下一页继续阅读##` 替换为空。

### 发现页（exploreUrl）
- **必须是 JSON 数组字符串**，每个元素包含 `title`、`url`（含 `{{page}}`）、`style` 字段。不要用换行分隔的 URL 字符串。详见 `references/legado-json-structure.md`。
