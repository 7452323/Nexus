---
name: js-reverse-mcp-integration
description: JavaScript逆向MCP集成技能。基于Patchright反检测引擎的JS调试与分析MCP服务器，支持23种工具：反检测浏览器、脚本分析、断点调试、函数追踪、运行时检查、网络分析、脚本注入。来自zhizhuodemao/js-reverse-mcp。
