...
基于 JSReverser-MCP 方法论提炼的本地 Node 补环境落地规范。目标不是"一次性模拟浏览器"，而是基于页面证据，让目标参数链路先跑通，再逐项补缺口。
...
主产物：`env/env.js` + `env/polyfills.js` + `env/entry.js`
...
主产物：`run/pure-*.js` + 可选 `run/pure_*.py` + 固定夹具与验收记录

进入条件：只有 env rebuild 跑通且服务端验收通过后才可进入。
...
- `watch` — Proxy 包装，记录 get/set/has/ownKeys
- `safeFunction` — 让 Function.prototype.toString 返回 native code
- `makeFunction` — 创建带诊断日志的函数壳
...
- 缺最小对象契约：只补该对象当前链路不可分割的最小接口集

"一次补一项"指的是"一次只做一个补丁决策"，而不是"永远只改一个属性名"。如果代理日志和 first divergence 已经证明当前缺口是同一个最小对象契约，可以一起补完该契约。

**每次补丁都应满足：**

- 能指出来源于哪条代理日志、哪条 first divergence 记录、哪条页面证据
...
|---------|------------|---------|-----------|
| navigator.userAgent、location.href 读取返回 undefined | 基础值缺失 | 直接补最小常量值 | 不要顺手补整套 navigator / location 伪实现 |
...
**职责：** 提供基础宿主、最小 storage shim、最小编码/加密壳

**禁止：** 访问日志、大量站点定制分支、直接内联目标业务脚本

### env/polyfills.js

**职责：** 代理诊断层、可复用函数伪装、缺函数壳和访问日志的辅助逻辑

**禁止：** 大量业务值硬编码、直接替代 env.js 作为基础宿主

### env/entry.js

**职责：** 先加载 env.js → 再加载 polyfills.js → 读 capture.json → 加载目标脚本 → 输出 first divergence 和当前可调用状态
...
如果后续要给 Python execjs 或其他宿主使用，建议额外导出：`run/exported-runtime.js`

如果后续要继续提纯为可读纯算法，建议再补：`run/pure-*.js`、可选 `run/pure_*.py`、固定夹具与服务端验收记录
